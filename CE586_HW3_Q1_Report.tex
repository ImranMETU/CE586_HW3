...
\subsection{OpenSeesPy Verification Workflow}

An independent OpenSeesPy verification workflow was prepared for the 5\% damped elastic response spectrum. The OpenSeesPy model uses a unit-mass linear elastic SDOF oscillator for each period and applies the DIN95Y01 record as base excitation. The detailed comparison between OpenSeesPy and the Python Newmark-beta implementation is presented in Chapter~7.
...
An important observation from Figure~\ref{fig:q2c_elastic_comparison} is that the 2018 TBDY DD-2 spectrum is generally lower than the 2007 TSC 475-year spectrum over most of the considered period range. This occurs because the 2007 spectrum is based on a broad seismic-zone approach with the first-degree seismic zone assumption, giving a short-period plateau of approximately $A_0 I S(T)=0.40(1.0)(2.5)=1.0g$. In contrast, the 2018 spectrum is obtained from coordinate-based AFAD TDTH parameters for the assigned Dinar site and ZC soil class, with $S_{DS}=0.764g$ for DD-2. Therefore, for this site and the assumptions adopted, the older 2007 zone-based spectrum is more conservative than the 2018 DD-2 spectrum in much of the period range. This result should not be interpreted as a general rule for all sites; it reflects the specific assigned coordinates, soil class, and the seismic-zone assumption used for the 2007 code.
...
The spectra in Figures~\ref{fig:q2d_r4} and~\ref{fig:q2d_r8} were obtained by applying the constant reduction $S_{a,R}=S_{a,e}/R$, as requested in the assignment. This procedure is appropriate as a simplified design-spectrum reduction and allows direct comparison between the record-based spectrum and the 2007/2018 design spectra for the same assumed response modification factor.

However, this constant reduction should not be interpreted as a complete nonlinear response-spectrum analysis. In classical inelastic response-spectrum theory, the relationship between elastic strength demand, yield strength, ductility demand, and period is period-dependent. In the medium- and long-period ranges, the equal-displacement approximation often gives a reasonable basis for relating ductility demand and strength reduction. In the short-period range, especially for periods smaller than the characteristic spectrum corner period, this approximation becomes less reliable. Very short-period systems tend to follow an equal-energy-type behavior, and a simple $1/R$ reduction may underestimate the strength required to achieve a target ductility demand. Therefore, the constant-$R$ spectra shown here should be understood as assignment-defined reduced design spectra, not as exact elastic-perfectly-plastic inelastic response spectra obtained from nonlinear time-history analysis.
...
In the reduced spectra, the record-based demand locally exceeds some of the 475-year code-based reduced spectra in selected short- and intermediate-period ranges, which indicates that even after constant-$R$ reduction a single near-field record may impose demands larger than the corresponding design spectrum over limited period bands. However, the record-based reduced spectrum generally remains below the 2475-year reduced design spectra.
% ------------------------------------------------------------
...
The comparison also shows that, under the first-degree seismic zone assumption, the 2007 475-year spectrum is generally more conservative than the site-specific 2018 DD-2 spectrum for this Dinar site. The constant-$R$ inelastic spectra provide the requested reduced design spectra, but the physical interpretation of strength reduction is period-dependent, particularly in the short-period range.
...
\subsection{OpenSeesPy Verification}

OpenSeesPy was used as an independent verification workflow for the 5\% damped elastic response spectrum. Each oscillator was modeled as a unit-mass linear elastic SDOF system with stiffness $k=\omega_n^2$ and damping ratio $\xi=5\%$. The DIN95Y01 record was applied as base excitation, and the transient response was computed using the Newmark constant average acceleration method with $\gamma=0.5$ and $\beta=0.25$.

The OpenSeesPy response spectrum was computed for all 29 periods between $0.2$ s and $3.0$ s and compared directly with the Python Newmark-beta implementation. The maximum absolute difference in $S_{a,\mathrm{actual}}$ was $0.000004g$, with a mean absolute difference of $0.000001g$. For the displacement spectrum $S_d$, the maximum absolute difference was $6.0\times10^{-6}$ m, with a mean absolute difference of $1.14\times10^{-6}$ m. The relative differences were effectively $0.00\%$.

This agreement confirms that the Newmark-beta implementation is independently verified against OpenSeesPy for the standard 5\% damping case.

\subsection{Sanitized OpenSeesPy Console Output}

\begin{Verbatim}[fontsize=\scriptsize]
======================================================================
OPENSEESPY VERIFICATION SCRIPT
CE586 Assignment 3 Q1 - Response Spectra
======================================================================

Step 1: Reading ground motion...
Found: C:\DDrive\Imran\METU\Coursework\CE586 - Earthquake\Assignment3\DIN95Y01.THF
Read 2796 points

Step 2: Computing response spectrum...

----------------------------------------------------------------------
OPENSEESPY VERIFICATION (Manual Time Stepping)
----------------------------------------------------------------------
Record: DIN95Y01
Time points: 2796
Duration: 27.95 s
...
OK Saved: C:\DDrive\Imran\METU\Coursework\CE586 - Earthquake\Assignment3\results\openseespy_verification_5percent.csv

Step 4: Comparing against Newmark results...

======================================================================
OPENSEESPY vs NEWMARK COMPARISON (xi = 5%)
======================================================================

----------------------------------------------------------------------
Sa_actual_g (Spectral Acceleration, g)
----------------------------------------------------------------------
Max absolute difference: 0.000004 g
Max relative difference: 0.00 %
Mean absolute difference: 0.000001 g
Mean relative difference: 0.00 %

----------------------------------------------------------------------
Sd_m (Displacement Spectrum, m)
----------------------------------------------------------------------
Max absolute difference: 6.000000e-06 m
Max relative difference: 0.00 %
Mean absolute difference: 1.137931e-06 m
Mean relative difference: 0.00 %

----------------------------------------------------------------------
Period-by-Period Comparison
----------------------------------------------------------------------
    T (s)     Newmark Sa    OpenSees Sa     Diff (%)
----------------------------------------------------------------------
      0.2       0.728665       0.728665         0.00
      0.3       1.259694       1.259694         0.00
      0.4       0.691168       0.691168         0.00
      0.5       0.643684       0.643688         0.00
      0.6       0.509077       0.509077         0.00
...
(opensees312) C:\DDrive\Imran\METU\Coursework\CE586 - Earthquake\Assignment3>
\end{Verbatim}

The assignment specification permits the use of external verification software such as SeismoSignal or OpenSeesPy. The results presented in this report are based on the Newmark-beta integration code developed in Python, which has been verified through independent comparison with OpenSeesPy as described above.
...
\begin{itemize}
    ...
    \item Displacement demand becomes increasingly significant in the longer-period range, with the 5\% damped spectrum reaching approximately $0.41$--$0.42\,\mathrm{m}$ near $T\approx2.0\,\mathrm{s}$.
...
The Python scripts \texttt{hw3\_response\_spectra.py}, \texttt{hw3\_q2\_design\_spectra.py}, and \texttt{verify\_openseespy\_response\_spectrum.py} are submitted together with this report. The first script computes the Newmark-beta response spectra for Question 1, the second script generates the 2007 and 2018 code-based spectra for Question 2, and the third script performs the OpenSeesPy verification of the 5\% damped elastic response spectrum. The OpenSeesPy verification results are saved in \texttt{results/openseespy\_verification\_5percent.csv}.
The code can be accessed at: \url{https://github.com/ImranMETU/CE586_HW3}
\end{document}
